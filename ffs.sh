...
reference=$(find "$1" \( -name "*.srt" ! -name "*bad.srt" -o -name "*.mkv" \) -print0 | xargs -0 -I {} basename "{}" | sort -t. -k2,2 -k1,1  | fzf --header="Pick a reference file, only one reference file can be choosen:")
